export const era47songs = {
        albumId: "era47",
        songsList: [
            {
                id: 1,
                songName: "WODA SODOWA",
                artists: ["Oki", "@atutowy"],
                streams: "11,903,855",
                duration: "2:35",
            },
        
            {
                id: 2,
                songName: "NIEWOLNIK",
                artists: ["Oki", "@atutowy"],
                streams: "4,494,218",
                duration: "2:03",
            },
        
            {
                id: 3,
                songName: "CERTIFIED LOVER",
                artists: ["Oki", "Bedoes 2115", "@atutowy"],
                streams: "6,972,241",
                duration: "2:39",
            },
        
            {
                id: 4,
                songName: "SPRZEDAŁEM SIĘ",
                artists: ["Oki", "Swizzy", "Worek", "Nolyrics Beats"],
                streams: "29,516,154",
                duration: "3:27",
            },
        
            {
                id: 5,
                songName: "PUK PUK PUK",
                artists: ["Oki", "Otsochodzi", "@atutowy"],
                streams: "3,267,782",
                duration: "2:35",
            },
        
            {
                id: 6,
                songName: "JEREMY SOCHAN SKIT",
                artists: ["Oki"],
                streams: "8115",
                duration: "0:19",
            },
        
            {
                id: 7,
                songName: "JEREMY SOCHAN",
                artists: ["Oki", "bvdy47"],
                streams: "41,626,131",
                duration: "2:25",
            },
        
            {
                id: 8,
                songName: "ŻYCIE TO ZA MAŁO",
                artists: ["Oki", "@atutowy"],
                streams: "4,589,096",
                duration: "2:18",
            },
        
            {
                id: 9,
                songName: "ILE LAT?",
                artists: ["Oki", "Sobel", "@atutowy"],
                streams: "33,133,571",
                duration: "2:30",
            },
        
            {
                id: 10,
                songName: "JESTEŚMY ZA MŁODZI",
                artists: ["Oki", "Oskar83", "@atutowy"],
                streams: "2,856,487",
                duration: "3:12",
            },
        
            {
                id: 11,
                songName: "NA ZAWSZE MAŁOLAT",
                artists: ["Oki", "@atutowy"],
                streams: "8,596,105",
                duration: "2:54",
            },
        
            {
                id: 12,
                songName: "OD ZERA",
                artists: ["Oki", "Blaki Selektah", "Deemz", "The Returners"],
                streams: "2,136,637",
                duration: "4:42",
            },
        
            {
                id: 13,
                songName: "JEŚLI CHCIAŁBYŚ TO MOŻESZ WPAŚĆ",
                artists: ["Oki", "Kuban", "@atutowy"],
                streams: "3,292,179",
                duration: "2:20",
            },
        
            {
                id: 14,
                songName: "WRAK",
                artists: ["Oki", "@atutowy"],
                streams: "9,270,730",
                duration: "3:19",
            },
        
            {
                id: 15,
                songName: "SEROTONINA",
                artists: ["Oki", "SHDØW"],
                streams: "2,118,593",
                duration: "2:23",
            },
        
            {
                id: 16,
                songName: "QUAD STUDIO FREESTYLE",
                artists: ["Oki", "bvdy47"],
                streams: "4,500,814",
                duration: "2:28",
            },
        
            {
                id: 17,
                songName: "KAUCJA",
                artists: ["Oki", "bvdy47"],
                streams: "16,520,787",
                duration: "2:27",
            },
        
            {
                id: 18,
                songName: "OK OK OK",
                artists: ["Oki", "@atutowy"],
                streams: "3,455,947",
                duration: "3:36",
        
            },
        
            {
                id: 19,
        
                songName: "DOM",
                artists: ["Oki", "Young Igi", "SHDØW"],
                streams: "2,057,737",
                duration: "3:30",
        
            },
        
        
            {
                id: 20,
                songName: "AGENT47",
                artists: ["Oki", "@atutowy"],
                streams: "14,868,229",
                duration: "2:47",
            },
        ]
}


export const okiAlbums = [
    {
        id: 1,
        cover: "/artists/oki/albums-covers/oki&nearr.jpg",
        title: "Oki & Nearr",
        authors: ["Oki", "Nearr"],
        releaseDate: "30.03.2018",
        duration: "0 30 44",
        songsList: [
            "Introdukcja",
            "Cześć, Oki",
            "Ślad",
            "Pokora",
            "Żyjemy",
            "Mam W Skit",
            "Mam W Nich",
            "Wstyd",
            "Fake",
            "Tjt"
        ],
        songsCount: 10
    },

    {
        id: 2,
        cover: "/artists/oki/albums-covers/47universe.jpg",
        title: "47universe",
        authors: ["Oki"],
        releaseDate: "25.03.2019",
        duration: "0 26 10",
        songsList: [
            "47universe Arrival",
            "Drug",
            "Bije W Serce",
            "Fre4sty7e",
            "Zimna Wodka",
            "Pokolenie 98",
            "Musze Zyc",
            "Whoop?",
            "Vestax"
        ],
        songsCount: 9
    },

    {
        id: 3,
        cover: "/artists/oki/albums-covers/47playground.jpg",
        title: "47playground",
        authors: ["Oki"],
        releaseDate: "10.01.2020",
        duration: "0 34 12",
        songsList: [
            "Ktoś czeka",
            "Voodoo",
            "Dysze",
            "Justin Bieber",
            "Na kodach",
            "Pull Up Faded",
            "Flashlight",
            "Kochać bloki",
            "Wracam do domu",
            "Tyle próśb"
        ],
        songsCount: 10
    },

    {
        id: 4,
        cover: "/artists/oki/albums-covers/77747mixtape.jpg",
        title: "77747mixtape",
        authors: ["Oki"],
        releaseDate: "25.09.2020",
        duration: "0 40 42",
        songsList: [
            "7747",
            "DLA ZIOMAL",
            "JEDEN DZIEŃ",
            "POLITYK",
            "ZA KAŻDĄ Z MOICH PRÓŚB",
            "NOWY CZEK",
            "SIRI",
            "SKIT O MARIHUANIE",
            "50 CENTÓW",
            "TAK BARDZO",
            "SUK",
            "NIE MARTW SIĘ MAMO",
            "OBRACAM KOŁEM",
            "TRUP"
        ],
        songsCount: 14
    },

    {
        id: 5,
        cover: "/artists/oki/albums-covers/oio.jpg",
        title: "OIO",
        authors: ["Oki", "Young Igi", "Otsochodzi", "OIO"],
        producers: ["@atutowy", "Kubi Producent", "Nolyrics Beats", "Phunk'ill", "CatchUp", "Sem0r", "Michał Anioł", "SHDØW"],
        releaseDate: "06.05.2021",
        duration: "0 45 0",
        songsList: [
            "Biggie",
            "Moonwalk",
            "Podzielony",
            "Jak To Ma Być",
            "Potłuczone Szkła",
            "Amerykańskie Teledyski",
            "Morze Łez",
            "Worki W Tłum",
            "MVP",
            "Przypadkiem",
            "5 AM",
            "Transparentny",
            "Trendsetting",
            "WMTB",
            "Paleta Barw"
        ],
        songsCount: 15
    },

    {
        id: 6,
        cover: "/artists/oki/albums-covers/produkt47.jpg",
        title: "PRODUKT47",
        authors: ["Oki"],
        releaseDate: "09.06.2022",
        duration: "00:47:44",
        songsList: [
            "I To Jest Fakt",
            "Perły",
            "Dzielny Pacjent",
            "Jeżyk!",
            "Albo Jest Zajebiście Albo Jest Smutno",
            "Stado Hartów",
            "Jakie To Uczucie?",
            "Sonic Skit",
            "ADHD",
            "Od Środy Do Wtorku",
            "Pieniądze, Dziewczyny, Zwrotki",
            "Fresh Water Soda",
            "Po Co Ten Foch?",
            "Doja Cat",
            "Bardziej Sobą Niż Kiedykolwiek",
            "Total Bandits Polne",
            "Zwariowane Polne"
        ],
        songsCount: 17
    },

    {
        id: 6,
        cover: "/artists/oki/albums-covers/era47.jpg",
        title: "ERA47",
        authors: ["Oki"],
        duration: "00:54:38",
        songsList: [
            "WODA SODOWA",
            "NIEWOLNIK",
            "CERTIFIED LOVER",
            "SPRZEDAŁEM SIĘ",
            "PUK PUK PUK",
            "JEREMY SOCHAN SKIT",
            "JEREMY SOCHAN",
            "ŻYCIE TO ZA MAŁO",
            "ILE LAT?",
            "JESTEŚMY ZA MŁODZI",
            "NA ZAWSZE MAŁOLAT",
            "OD ZERA",
            "JEŚLI CHCIAŁBYŚ TO MOŻESZ WPAŚĆ",
            "WRAK",
            "SEROTONINA",
            "QUAD STUDIO FREESTYLE",
            "KAUCJA",
            "OK OK OK",
            "DOM",
            "AGENT47"
        ],
        songsCount: 20
    },
]